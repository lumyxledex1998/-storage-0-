...
module.exports = {
    name: "anti-evento",
    description: "Ativa/Desativa o recurso de anti-evento no grupo.",
    commands: ["anti-evento", "anti-event"],
    usage: `${PREFIX}anti-evento (1 / 0)`,
    handle: async ({
        remoteJid,
        isGroup,
        args,
        ...
        try {
            ...
            if (!admins.includes(userJid)) {
                return await sendWarningReply(
                    "Apenas administradores podem configurar o anti-evento!"
                );
            }
            ...
            restrictions[remoteJid]["anti-event"] = enable;
            ...
            return await sendSuccessReply(
                `Anti-evento ${status} com sucesso!`
            );
        } catch (error) {
            console.error("Erro no comando anti-evento:", error);
            return await sendErrorReply(
                "Ocorreu um erro ao configurar o anti-evento. Por favor, tente novamente."
            );
        }
    }
};
