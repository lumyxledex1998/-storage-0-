...
exports.messageHandler = async (socket, webMessage) => {
    try {
        ...
        const messageType = Object.keys(readRestrictedMessageTypes()).find(
            type => getContent(webMessage, type)
        );
        ...
        await socket.sendMessage(
            remoteJid,
            {
                text: `⚠️ ${pushName} foi removido por enviar ${messageType}`
            },
            { quoted: webMessage }
        );
    } catch (error) {
