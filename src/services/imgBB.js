...
exports.imgbbUpload = async (buffer, options = {}) => {
  try {
    ...
    if (!response.data.success) {
      throw new Error(response.data.error?.message || 'Falha no upload para o ImgBB');
    }
    ...
    errorLog(`Erro ao fazer upload da imagem no ImgBB:  ${JSON.stringify(error, null, 2)}`);
    
    if (error.response) {
      const apiError = error.response.data?.error || {};
      throw new Error(`ImgBB API Error: ${apiError.message || 'Unknown error'} (Code: ${apiError.code || 'unknown'})`);
